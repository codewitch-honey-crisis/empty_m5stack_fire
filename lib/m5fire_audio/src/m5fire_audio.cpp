...
bool m5fire_audio_shape(unsigned shape,float frequency, float volume) {
    struct start_data {
        float frequency;
        float volume;
    };
...
void m5fire_audio_sin(m5fire_audio_queue_message& msg) {
    ...
    while(!xQueueReceive(m5fire_audio_queue,&msg,0)) {
        ...
        for (int i=0; i < DMA_BUF_LEN; i++) {
            ...
            m5fire_audio_out_buf[i] = (uint8_t)samp ;//<< 16;
        }
        // time critical
...
bool m5fire_audio::initialize() {
    if(!m5fire_audio_initialized) {
        ...
        xTaskCreate(m5fire_audio_task_fn, "m5fire_audio", 1024*4, nullptr, configMAX_PRIORITIES - 1,&m5fire_audio_task);
...
bool m5fire_audio::sinw(float frequency,float volume = 1.0) {
    return m5fire_audio_shape(0,frequency,volume);
}
